...
class PresentationController extends Controller {
    ...
    public function create(Request $request) {
        ...
        //upload video
        $uploadedFile = $request->file('filevid');
        $filename = $uploadedFile->getClientOriginalName();
        $uFileName = time() . '-' . $filename;
        Storage::disk('public_uploads')->putFileAs('videos', $uploadedFile, $uFileName);

        $user = Presentation::create(array(
                    'name' => $request['txttitle'],
                    'clues' =>  json_encode($answersArray),
                    'file' => 'uploads/videos/' . $uFileName,
        ));
